...
<form action="consignermasterentry" method="post">
  <div class="form-row">
    <div class="form-group col-md-4">
      <label name = "Consigner Name">Consigner Name</label>
      <input type="text" class="form-control" placeholder="Enter Consigner Name" name="c_name" required>
    </div>
    <div class="form-group col-md-4">
      <label for="Consigner Code">Consigner Code</label>
      <input type="text" class="form-control" placeholder="Enter Consigner Code" name="c_code" required>
    </div>
    <div class="form-group col-md-4">
      <label for="Address">Address</label>
      <input type="text" class="form-control" placeholder="Enter Address" name="c_address" required>
    </div>
  </div>
  <div class="form-row">
    <div class="form-group col-md-6">
      <label for="Contact No">Contact No.</label>
      <input type="text" class="form-control" placeholder="Enter Contact No." name="c_no" required="">
    </div>
        <div class="form-group col-md-6">
      <label for="Gst No">Gst No.</label>
      <input type="text" class="form-control" placeholder="Enter Gst No." name="c_gno" required="">
    </div>
  </div>
  <center>
...
<br><br>
<% 
     List<List> consigners = new ArrayList<List>();

     if (request.getSession().getAttribute("consigners") != null) {
         consigners = (ArrayList ) request.getSession().getAttribute("consigners");
     }
     Set <String> engaged_consigners = new HashSet<>();
     if (request.getSession().getAttribute("engaged_consigners") != null) {
         engaged_consigners = (HashSet) request.getSession().getAttribute("engaged_consigners");
     }
%>
<div class="container"> 
...
<%
	for(List<String> row:consigners)
	{%>
		<tr>
			<td><font color="black"><%=row.get(1)%></font></td>
			<td><font color="black"><%=row.get(0)%></font></td>
			<td><font color="black"><%=row.get(2)%></font></td>
			<td><font color="black"><%=row.get(3)%></font></td>
		<td><a href= "updateconsigner?c_id=<%=row.get(4)%>"><button type="button" class="update">Update</button></a>
			&nbsp;
			<%
			if(!engaged_consigners.contains(row.get(4)))
			{%>
				<a href= "deleteconsigner?c_id=<%=row.get(4)%>"><button type="submit" class="delete" onclick="return isSure();">Delete</button></a>
			<%}%>
		</td>
		</tr>
	<%}

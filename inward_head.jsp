...
<form action="inwardheadentry" method="post">
  <div class="form-row">
    <div class="form-group col-md-6">
      <label name = "Transport Name">Transport Name</label>
      <input type="text" class="form-control" placeholder="Enter Transport Name" name="ts_name" id="ts_name" required>
    </div>
    <div class="form-group col-md-6">
      <label for="From Station">From Station</label>
      <input type="text" class="form-control" placeholder="Enter From Station" name="fr_station" id="fr_station" required>
    </div>
   </div>
   <div class="form-row">
   	 <div class="form-group col-md-4">
      <label for="Dispatch Date">Dispatch Date</label>
      <input type="date" class="form-control" placeholder="Enter Dispatch Date" name="ds_date" id="ds_date" required>
    </div>
    <div class="form-group col-md-4">
      <label for="Challan No.">Challan No.</label>
      <input type="text" class="form-control" placeholder="Enter Challan No." name="ch_no" id="ch_no" required="">
    </div>
        <div class="form-group col-md-4">
      <label for="Challan Date">Challan Date</label>
      <input type="date" class="form-control" placeholder="Enter Challan Date" name="ch_date" id="ch_date" required="">
    </div>
   </div>
   <div class="form-row">
   	<div class="form-group col-md-4">
      <label for="Lorry No.">Lorry No.</label>
      <input type="text" class="form-control" placeholder="Enter Lorry No." name="lorry_no" id="lorry_no" required>
    </div>
    <div class="form-group col-md-4">
      <label for="Arrival Date">Arrival Date</label>
      <input type="date" class="form-control" placeholder="Enter Arrival Date" name="av_date" id="av_date" required="">
    </div>
    <div class="form-group col-md-4">
      <label for="Total Bales">Total Bales</label>
      <input type="number" class="form-control" placeholder="Enter Total Bales" name="total_bales" id="total_bales" required="">
    </div>
  </div>
  <center>
  &nbsp;&nbsp;&nbsp;
	<button type="submit" id="addbtn" class="btn btn-secondary" value="Add">Add</button>
	<button type="submit" id="updatebtn" class="btn btn-secondary" value="Update" hidden="">Update</button>
...
<% 
     ...
     if (request.getSession().getAttribute("inwardhead") != null) {
         inward = (ArrayList ) request.getSession().getAttribute("inwardhead");
     }
     Set <String> engaged_inward = new HashSet<>();
     if (request.getSession().getAttribute("engaged_inward") != null) {
         engaged_inward = (HashSet) request.getSession().getAttribute("engaged_inward");
     }
%>
<div class="container"> 
...
<table class = "table">
  <thead class="thead-dark">
    <tr>
      ...
      <th scope="col">Edit Inward Details</th>
    </tr>
  </thead>
  <%
  int i=-1;
...
<%
	...
	for(List<String> row:inward)
	{
		...
		<tr>
			...
			<%String arvDate=row.get(7);
			String []dtarr=arvDate.split("-");
			String dd=dtarr[2];
			String mm=dtarr[1];
			String yyyy=dtarr[0];
			String actDate=dd+"/"+mm+"/"+yyyy;
			%>
			<td><font color="#000"><%=actDate%></font></td>
			<td><font color="#000"><%=row.get(8)%></font></td>
			<td><a href= "updateinward?inward_id=<%=row.get(0)%>"><button type="button" class="update" onclick="return update_it();">Update</button></a>
				&nbsp; 
			<%if(!engaged_inward.contains(row.get(0)))
			{%>
				<a href= "deleteinward?inward_id=<%=row.get(0)%>"><button type="button" onclick="return isSure();" class="update">Delete</button></a>
			<%}%>
			</td>
		<td><a href= "editinwarddetails?ch_no=<%=row.get(4)%>"><button type="button" class="update">Edit Inward Details</button></a>
		</td>
		</tr>
		<%}
%>
